...
**Code.** Lean `Expr`/`eval` now has `letE` / `andE` / `orE` / `cons` in `proof/ShenLogic.lean`. `ite` / `andE` / `orE` use Shen's `= true` (any other *value* is else / false / rhs; `none` stays stuck). `letE` shadows on the front of `ρ`. `cons` is strict L-to-R: a `.list` tail becomes `.list (h :: xs)`; any other tail is `.ctor "cons" [h, t]`. That last clause is a model of improper cons, not a T(P) axiom and not Lean `Value.list` injectivity. `eval_deterministic` is still `some`-injectivity. No adequacy lemma. No `v2-control.slir` / `control.graph.logic` golden.
