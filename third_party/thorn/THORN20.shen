(package thorn [prop term <=> => ~ v & exists all f c x compiled kb-> <-kb unix attach]
...
(defprolog attach
  (- true)  Procedure Hyp Depth Max Prf <-- (when (eval Procedure));
  (- false) Procedure Hyp Depth Max Prf <-- (when (not (eval Procedure)));)             
...
(defprolog solve                            
  ...
  P Prf                                <--     (bind Goals (literals P))
                                               (bind Hyp (map (fn complement) Goals))
                                               (hypdisj Goals Hyp P Prf);)
...
(define literals
  [P v Q] -> (union (literals Q) (literals P))
  P -> [P]) 
  
(defprolog hypdisj                                       
   [Goal | Goals] Hyp Disjunction [[hypdisj Goal] | Prf] 
      <-- (show [] Disjunction "hypdisj" 0)
          (iterative-deepening (map (fn sign) (remove (complement Goal) Hyp)) 
                               (sign Goal) Prf 0);
   (- [_ | Goals]) Hyp Disjunction Prf <-- (hypdisj Goals Hyp Disjunction Prf);)
...
(define kb->
  Ps -> (let Equality?       (if (> (occurrences = Ps) 0) (equality true) (equality false))
             DeSugar         (desugar [(refl=) | Ps])
             Predicates      (remember-predicates DeSugar)
             NoProgram       (remove-if (fn contrapositive?) DeSugar)
             Program         (remove-if (/. P (not (contrapositive? P))) DeSugar)
             Clauses         (mapcan (fn clauses) NoProgram)
             Horn            (set *Horn?* (every? (fn horn?) Clauses))
             Contrapositives (append Program (mapcan (fn contrapositives) Clauses))
...
(define predicates-in
   ...
   [~ P]        -> (predicates-in P)
   [P <-- | Ps] -> (union (predicates-in P) (mapf (fn predicates-in) Ps (fn union)))
   [F | _]      -> [F]
   _            -> [])
...
(defprolog rewriteA+B
   ...
   F Fn Boolean Args (- [X | Y]) Hyps Depth Max [[rewriteA X Sub] [fork Prf1 Prf2]] <--  (not-var? X)
                                                                                         ...
                                                                                         (not-is! X Z) 
                                                                                         (sub Z X Args Sub)
                                                                                         (shen.callrec (Fn Boolean) (append Sub [Hyps Depth Max Prf2]));
